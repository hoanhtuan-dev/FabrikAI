...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // ── Luôn cần: cấu hình + tài khoản quản trị ──
        $this->settings();
        $this->users();
        ...
        // ── Dữ liệu DEMO của storefront: chỉ dev/test (production phải bật SEED_DEMO_DATA=true) ──
        if (! $this->demoDataEnabled()) {
            $this->command?->warn(
                'Production: ĐÃ BỎ QUA dữ liệu demo (danh mục/sản phẩm/đơn hàng/dự án mẫu). '.
                'Đặt SEED_DEMO_DATA=true nếu thật sự muốn. Tài khoản quản trị KHÔNG bị ảnh hưởng.'
            );
    ...
    {
        // ── Tài khoản QUẢN TRỊ: dùng firstOrCreate, KHÔNG BAO GIỜ ghi đè mật khẩu đã có ──
        // Đây là điểm quan trọng: bản cũ dùng updateOrCreate + mật khẩu hardcode, nên (a) mật khẩu
        // đã bị lộ vì repo PUBLIC, và (b) chạy lại seeder sẽ RESET mật khẩu về giá trị đã lộ đó.
        // firstOrCreate chỉ tạo khi tài khoản CHƯA tồn tại; người vận hành đổi mật khẩu rồi thì
        // seeder không đụng tới nữa.
        //
        ...
        // ── Dữ liệu DEMO (khách hàng mẫu) — không đổ vào production trừ khi bật SEED_DEMO_DATA ──
        if (! $this->demoDataEnabled()) {
            return;
        }
