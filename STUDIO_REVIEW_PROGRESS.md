...
- **Cứu model văn bản + lưu nháp** (5281b30): đo thật trên production — deepseek-chat/flash chạy ~1s (key hợp lệ), ckey sypham98 chạy 21s (gây 504), qwen 2 key đang TẮT. Đã tắt 4 model ckey text + trỏ translate → deepseek-chat (script production). Thêm lưu nháp localStorage cho Agent Studio (prompt/bước/trend/size). Suite 959 XANH · đã deploy HEAD 5281b30.
- **Còn nợ (chủ dự án)**: nạp key Qwen (cho ảnh/vision/model văn bản) · kiểm tra api.xah.io · bật cron+queue · thêm nguồn RSS · xác minh vai tìm kiếm bằng model search thật · test frontend (Vitest).

- **Tool search cho vai Tìm kiếm** (2026-09-24): khảo sát phát hiện nút thắt nằm ở CƠ CHẾ chứ không ở Cài đặt — `WebAccessService::planFor()` chỉ biết 2 đường (qwen/dashscope `enable_search` · gemini `google_search`) + Custom Provider tự khai `search_param`; giao thức **openai** (DeepSeek — model văn bản đang chạy trên production) KHÔNG có đường nào ⇒ gán model vào nhóm `agent_search` vẫn không tìm được gì và lượt chạy lặng lẽ quay về nhóm suy luận. Đã thêm **tầng thứ ba: CÔNG CỤ do MÁY CHỦ chạy** — `WebSearchTool` khai hàm `web_search` theo chuẩn function-calling, `AiModelGateway` chạy vòng lặp (gọi hàm → máy chủ tìm thật → trả `role:"tool"` → model viết JSON), `WebSourceService::search()` tìm theo từ khoá trên nguồn có `{query}` hoặc sẵn `q=` (nguồn Google News mặc định thành nguồn tìm kiếm NGAY, không phải migrate). Ba ràng buộc khoá bằng test: chỉ bật khi vai `agent_search` được gán model (không đội thời gian chờ của mọi lượt chạy) · provider từ chối `tools` thì gọi lại đường thường VÀ nói thật `accepted=false` (không hứa suông) · trần `MAX_CALLS=5` mỗi lần thử, cấp lại khi tầng gọi thử lại vì JSON bị cắt. **[ĐÍNH CHÍNH 2026-09-26: bản ghi này chép "3"; mã hiện tại là 5 — `app/Services/WebSearchTool.php:40`, chú thích "2026-09-21: nâng 3 → 5". Không viết lại phần còn lại của bản ghi.]** Giao diện đọc **số đo** `model.tool_search` (đã tìm mấy lượt · từ khoá gì · bao nhiêu tin · nguồn nào) thay cho câu văn tĩnh, thêm verdict `internet_and_tool_search`, và sửa câu nói sai *"không phải model tự tìm kiếm"*. Đo THẬT: từ khoá «áo dạ tweed» → **586 ms · 1 tin trong 60 ngày · có URL + thời điểm**, hỏi lại 10 ms (đệm theo nguồn · từ khoá). `ToolSearchTest` 10 test · `WebSourceTest` 13→19 · `AgentRolesTest` 12→13 · `BrandDnaTest` +1 · **suite 978 test / 6.974 assert XANH** · `docs/DESIGN_SYSTEM.md` §18.2 thêm tầng thứ ba. **Việc chủ dự án**: gán model cho «Agent Studio — Tìm kiếm nguồn ngoài» trong Cài đặt → Nhóm công việc (production: `deepseek:deepseek-chat`) rồi bấm «Kiểm tra lại» ở bước Tín hiệu.
