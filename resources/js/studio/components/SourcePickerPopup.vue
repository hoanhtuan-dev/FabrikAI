  ...
  <div v-if="modelValue" role="dialog" aria-modal="true" aria-label="Chọn nguồn ảnh" class="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4" @click.self="close">
    <div class="flex h-[82vh] w-full max-w-3xl flex-col rounded-lg border border-ink-700 bg-ink-900 p-4 shadow-2xl" style="height: min(82vh, 760px)">
      ...
      <div class="mb-3 flex items-start justify-between">
        <div class="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p class="text-[11px] text-cream-400">Thư viện {{ refs.length + output.length }} ảnh — nhấn chọn nhiều rồi thêm vào canvas</p>
          </div>
        </div>
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        ...
        <p v-if="loadError" class="mb-2 flex items-center gap-1.5 rounded-md border border-red-500/40 bg-red-500/10 px-2.5 py-1.5 text-[11px] text-danger">
        ...
        <div class="scrollbar-hide -mr-1 min-h-0 flex-1 overflow-y-auto overscroll-contain pr-1">
          ...
          <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center">
            <div class="relative flex-1">
              ...
              <input v-model="query" placeholder="Tìm theo tên ảnh…" class="h-9 w-full rounded-md border border-ink-700 bg-ink-800/60 pl-9 pr-3 text-xs text-cream-100 placeholder:text-cream-400 focus:border-brand-500 focus:outline-none" aria-label="Tìm ảnh theo tên">
            </div>
            <div class="relative">
