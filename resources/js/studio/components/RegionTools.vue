...
function pickMask(id) {
  store.selectTool = false; store.startCanvasSelect(id);
  store.filmOpen = false;
}
</script>
<template>
  ...
  <div class="pointer-events-none absolute inset-x-0 bottom-16 z-40 flex justify-center lg:inset-auto lg:left-2 lg:top-1/2 lg:bottom-auto lg:-translate-y-1/2 lg:flex-col">
    ...
    <div class="pointer-events-auto flex items-center gap-1 rounded-xl border border-ink-700 bg-ink-900/95 p-1 shadow-xl backdrop-blur lg:hidden">
      <button @click="store.selectTool = !store.selectTool; store.panMode = false; store.filmOpen = false; store.clearInpaintMask()"
      ...
      <button @click="store.panMode = !store.panMode; store.selectTool = false; store.filmOpen = false; store.clearInpaintMask()"
      ...
      <button @click="store.selectTool = false; store.filmOpen = !store.filmOpen; store.clearInpaintMask()"
