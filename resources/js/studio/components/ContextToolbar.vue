...
const pillL = 'flex items-center gap-1 pr-1 text-tiny text-cream-300';
const sld = 'h-1 min-w-0 flex-1 cursor-pointer accent-cream-300';
const val = 'w-7 shrink-0 text-right text-micro tabular-nums text-cream-100';
...
<template>
  ...
  <div v-else-if="store.inpaintMaskDone" :class="[bar, ring]">
    <span class="flex items-center gap-1 px-1 text-label font-medium text-cream-200"><StudioIcon name="check" :size="I"/>Đã lưu vùng</span>
  ...
  <div v-else-if="store.filmOpen || store.looking" :class="[bar, ring]">
    ...
    <button v-for="p in looks" :key="p[0]" type="button" @click="store.lookPreset = p[0]" class="rounded-full border px-2 py-0.5 text-body font-medium transition-colors" :class="store.lookPreset === p[0] ? chipOn : chipOff">{{ p[1] }}</button>
    <span class="h-4 w-px bg-ink-600"></span>
    <div class="flex items-center justify-center gap-0.5 rounded-lg bg-ink-800/70 px-1.5 py-0.5"><StudioIcon name="sliders" size="h-3 w-3" class="text-brand-300"/><input type="range" min="0" max="10" step="1" v-model.number="store.lookLevel" class="h-1 w-12 cursor-pointer accent-cream-300"><span class="w-8 text-right text-tiny tabular-nums text-cream-100">{{ store.lookLevel }}/10</span></div>
  ...
  <div v-else-if="store.activeLayer && !store.selectTool" :class="[bar, ring]">
    ...
    <span :class="sep"></span>
    <!-- Số layer đang chọn -->
    <span class="flex items-center gap-1 rounded-full bg-ink-800 px-2 py-0.5 text-label text-cream-300"><StudioIcon name="layers" size="h-3 w-3" class="text-brand-300" />{{ store.selectionUnitCount || 1 }}</span>
  </div>

  <!-- ══ Select Tool: marquee chọn hàng loạt + chọn tất cả / bỏ chọn + fit selection ══ -->
  <div v-else-if="store.selectTool" :class="[bar, ring]">
    <span class="flex items-center gap-1 px-1 text-label font-medium text-cream-200"><StudioIcon name="cursor" :size="I"/>Quét chọn</span>
    ...
    <button @click="store.zoomFitSelection()" :class="[lbl, btn]" title="Fit canvas — đưa các layer/group đang chọn vào giữa khung nhìn"><StudioIcon name="maximize" :size="I"/>Fit chọn</button>
    <span :class="sep"></span>
    <span class="flex items-center gap-1 rounded-full bg-ink-800 px-2 py-0.5 text-label text-cream-300"><StudioIcon name="layers" size="h-3 w-3" class="text-brand-300" />{{ store.selectionUnitCount || (store.activeLayer ? 1 : 0) }}</span>
  </div>

  <!-- ══ Pan tool: context bar gọn với zoom + fit ══ -->
  <div v-else-if="store.panMode" :class="[bar, ring]">
    <span class="flex items-center gap-1 px-1 text-label font-medium text-cream-200"><StudioIcon name="hand" :size="I"/>Di chuyển canvas</span>
    ...
    <button @click="store.zoomFit()" :class="[lbl, btn]" title="Fit canvas — toàn bộ layer vừa khung nhìn"><StudioIcon name="maximize" :size="I"/>Fit toàn bộ</button>
  </div>

  <!-- ══ No active tool: placeholder text ══ -->
  <span v-else class="shrink-0 whitespace-nowrap text-body font-medium text-cream-400">Chọn công cụ từ thanh công cụ cạnh canvas (Esc = hủy · Enter = xong)</span>
</template>