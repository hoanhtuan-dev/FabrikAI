...
import { useTheme, FONT_OPTIONS } from '../../composables/useTheme.js';
 ...
 */
const { pref, options, fontScale, setTheme, setFontScale } = useTheme();
const busy = ref(false);
const fontBusy = ref(false);

/**
 * Cỡ chữ toàn cục — cùng cách xử lý như theme: áp NGAY (không có nút Lưu), lưu theo TÀI KHOẢN.
 * Mức là PHẦN TRĂM nhân vào token --text-* nên mọi bậc chữ trong app to/nhỏ cùng nhau; bố cục
 * (padding, kích thước nút) giữ nguyên nên không phá vỡ các hàng chật trong Studio.
 */
async function chooseFont(pct) {
  if (fontBusy.value || fontScale.value === pct) return;
  fontBusy.value = true;
  try {
    const res = await setFontScale(pct);
    if (res && res.saved) notify.ok('Đã đổi cỡ chữ và lưu vào tài khoản.');
    else if (res && res.error) notify.err(res.error);
    else notify.ok('Đã đổi cỡ chữ cho lần dùng này.');
  } finally {
    ...
    <div class="mt-3 grid gap-2 sm:grid-cols-3" role="radiogroup" aria-label="Chọn giao diện">
      <button v-for="opt in options" :key="opt.id" type="button" role="radio"
        ...
        <span class="min-w-0 flex-1">
          ...
          <span class="mt-1 block text-label leading-snug text-cream-300">{{ opt.desc }}</span>
        </span>
      </button>
    </div>

    <!-- ── CỠ CHỮ TOÀN CỤC ──────────────────────────────────────────────────────────────── -->
    <h3 class="mt-5 font-display text-sm font-semibold text-base-content">Cỡ chữ</h3>
    <p class="mt-1 text-body leading-relaxed text-cream-300">
      Chỉnh cỡ chữ cho TOÀN BỘ giao diện (cả Studio lẫn các trang khác). Mức "Vừa" đã to hơn bản trước
      một bậc; chọn mức khác là thấy đổi ngay. Đây là cỡ chữ của sản phẩm, không phải mức phóng to của
      trình duyệt — nên nó đi theo tài khoản sang máy khác.
    </p>

    <div class="mt-3 grid gap-2 sm:grid-cols-4" role="radiogroup" aria-label="Cỡ chữ">
      <button v-for="f in FONT_OPTIONS" :key="f.id" type="button" role="radio"
              :aria-checked="fontScale === f.id ? 'true' : 'false'"
              :disabled="fontBusy"
              @click="chooseFont(f.id)"
              :class="fontScale === f.id
                ? 'border-brand-500 bg-primary/15 text-base-content'
                : 'border-ink-600 bg-base-100 text-base-content hover:border-brand-400'"
              class="motion-ui flex flex-col items-start gap-1 rounded-lg border p-3 text-left disabled:opacity-60">
        <span class="flex items-center gap-1.5 text-sm font-semibold">
          <span aria-hidden="true" :style="{ fontSize: (f.id / 100) + 'rem' }">Aa</span>
          {{ f.label }}
          <StudioIcon v-if="fontScale === f.id" name="check" size="h-3.5 w-3.5 shrink-0" />
        </span>
        <span class="text-label leading-snug text-cream-300">{{ f.id }}% · {{ f.desc }}</span>
      </button>
    </div>
