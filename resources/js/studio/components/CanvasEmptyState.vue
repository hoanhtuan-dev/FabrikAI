          ...
          <section class="motion-rise-in rounded-2xl border border-ink-700 bg-ink-900/90 p-4 shadow-2xl sm:p-5">
            ...
            <div class="relative mt-3">
              ...
              <div class="absolute bottom-3 right-3 flex items-center gap-2">
                <span class="text-label font-semibold text-cream-400" title="Số credit ước tính">~{{ creditEstimate }} credit</span>
          ...
          <section class="motion-rise-in rounded-2xl border border-ink-700 bg-ink-900/80 p-4">
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p class="mt-0.5 text-body text-cream-400">Agent Studio dẫn từ tín hiệu thị trường đến prompt tạo ảnh.</p>
              </div>
            ...
            <div class="mt-3 grid gap-2 sm:grid-cols-3">
              <div v-for="(stepItem, index) in AGENT_STEPS" :key="stepItem.id" class="rounded-xl border border-ink-700 bg-ink-800/70 p-3">
                <div class="flex items-center gap-2">
                  <span class="grid h-6 w-6 place-items-center rounded-full bg-ink-900 text-label font-bold text-brand-200">{{ index + 1 }}</span>
                ...
                <p class="mt-2 text-body leading-4 text-cream-400">{{ stepItem.text }}</p>
              </div>
            </div>
          </section>
        </div>
        ...
        <aside class="space-y-4">
          <section class="motion-rise-in rounded-2xl border border-ink-700 bg-ink-900/80 p-4">
            ...
            <div class="mt-3 space-y-2">
              <button v-for="action in quickActions" :key="action.id" type="button" class="tool-btn w-full justify-start !px-3 !py-2.5 text-left" :title="action.hint" @click="action.run()">
                ...
                <span class="min-w-0"><span class="block text-xs font-semibold text-cream-100">{{ action.label }}</span><span class="block truncate text-label text-cream-400">{{ action.hint }}</span></span>
              </button>
              <button type="button" class="tool-btn w-full justify-start !px-3 !py-2.5 text-left" title="Xem gói và nâng cấp" @click="store.planOpen = true">
                ...
                <span class="min-w-0"><span class="block text-xs font-semibold text-cream-100">Gói &amp; credit</span><span class="block truncate text-label text-cream-400">Hạn mức, chi phí, nâng cấp</span></span>
              </button>
            </div>
          </section>
          ...
          <details class="motion-rise-in rounded-2xl border border-ink-700 bg-ink-900/80 p-4">
            ...
            <ul class="mt-3 space-y-1.5">
              <li v-for="item in shortcuts" :key="item.key" class="flex items-center justify-between gap-3 text-body text-cream-400">
                <span>{{ item.label }}</span>
                <kbd class="rounded bg-ink-800 px-1.5 py-0.5 font-mono text-label text-cream-200">{{ item.key }}</kbd>
              </li>
            </ul>
          </details>
        </aside>
