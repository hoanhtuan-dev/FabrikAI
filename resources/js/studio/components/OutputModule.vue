 ...
 *
 * Nguyên tắc 5 của docs/DESIGN_SYSTEM.md §12: "Kết quả luôn có bước tiếp theo — Biến thể · Sửa · Ghép ·
  ...
  <div class="card flex flex-1 flex-col overflow-hidden" style="min-height:0">
    ...
    <div class="scrollbar-hide mt-2 grid flex-1 auto-rows-min grid-cols-1 gap-1.5 overflow-y-auto p-2">
      <div v-for="g in store.visibleGenerations" :key="g.id" class="group relative aspect-square overflow-hidden rounded-lg border-2" :class="store.previewId === g.id ? 'border-brand-500' : 'border-ink-700'">
        <!-- Badge dự án -->
        <span v-if="g.project_id" class="absolute top-1 left-1 z-10 h-2.5 w-2.5 rounded-full ring-1 ring-ink-600" :style="{ background: projectColor(g.project_id) }" :title="'Dự án: ' + projectName(g.project_id, g.project)"></span>
        ...
        <span v-if="g.is_demo" class="absolute right-1 top-1 z-10 rounded-full bg-warn px-1.5 py-0.5 text-micro font-bold uppercase leading-none text-warn-content" :title="g.demo_reason || 'Ảnh mẫu — tính năng tạo ảnh AI chưa được bật'">ẢNH MẪU</span>
        <!-- Ảnh hoàn tất -->
        <template v-if="g.status === 'completed' && g.media_url">
          <button @click="store.openViewer(g)" draggable="true" @dragstart="onThumbDrag($event, g)" class="absolute inset-0 cursor-grab active:cursor-grabbing" :title="'Kéo thả vào canvas để thêm · nhấn để xem lớn'"><img :src="thumbUrl(g.media_url, 320)" class="pointer-events-none h-full w-full bg-ink-900 object-cover" loading="lazy" @error="onThumbError($event, g.media_url)"></button>
          ...
          <div class="pointer-events-none absolute inset-x-0 bottom-0 z-20 grid grid-cols-2 gap-0.5 bg-scrim/80 p-0.5 opacity-0 backdrop-blur-sm transition group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100">
            <button type="button" class="flex h-6 items-center justify-center gap-1 rounded bg-scrim/85 text-tiny font-semibold text-scrim-content transition hover:bg-brand-600 hover:text-cream-50" title="Thêm vào canvas (thành layer để ghép/sửa)" :aria-label="'Thêm ' + store.genName(g) + ' vào canvas'" @click.stop="toCanvas(g)">
              <StudioIcon name="plus" size="h-3 w-3" /> Canvas
            </button>
            <button type="button" class="flex h-6 items-center justify-center gap-1 rounded bg-scrim/85 text-tiny font-semibold text-scrim-content transition hover:bg-brand-600 hover:text-cream-50" title="Tải ảnh gốc về máy" :aria-label="'Tải ' + store.genName(g)" @click.stop="download(g)">
              <StudioIcon name="download" size="h-3 w-3" /> Tải
            </button>
            <button type="button" class="flex h-6 items-center justify-center gap-1 rounded bg-scrim/85 text-tiny font-semibold text-scrim-content transition hover:bg-brand-600 hover:text-cream-50" title="Tạo biến thể từ ảnh này (mở công cụ Biến thể)" :aria-label="'Tạo biến thể từ ' + store.genName(g)" @click.stop="useIn(g, 'variation')">
              <StudioIcon name="variations" size="h-3 w-3" /> Biến thể
            </button>
            <button type="button" class="flex h-6 items-center justify-center gap-1 rounded bg-scrim/85 text-tiny font-semibold text-scrim-content transition hover:bg-brand-600 hover:text-cream-50" title="Sửa ảnh này (mở công cụ Sửa ảnh)" :aria-label="'Sửa ' + store.genName(g)" @click.stop="useIn(g, 'inpaint')">
